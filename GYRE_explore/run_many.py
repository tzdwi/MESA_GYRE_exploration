...
if __name__ == "__main__":
    ...
    profile_files = glob('../25M_mesa/LOGS_postrsg/profile*.data.GSM')
    ...
    profiles = np.arange(*profile_min_max,10,dtype=int)
    
    for n in profiles:
        if f'25M/summary_{n}.h5' in glob('25M/summary*'):
            print(f'Already run profile {n}')
            continue
        with open("25M_gyre_template.txt", "r") as text_file:
